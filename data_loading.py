...
def get_data(args, data_config, run_local=False):
    ...
    if run_local:
        ...
        transaction_file = f"{data_config['paths']['aml_data']}/{args.data}/formatted_transactions_5_hrs.csv"
    else:
    ...
    print('read from file: ', transaction_file)
    ...
    # If we're in run_local (5-hour test CSV), split into 1-hour buckets;
    # otherwise keep the default 24-h “days.”
    if run_local:
        unit = 3600        # one hour
    else:
        unit = 3600 * 24   # one day
    n_days = int(timestamps.max() / unit) + 1
    n_samples = y.shape[0]
    # logging.info(f'number of days and transactions in the data: {n_days} days, {n_samples} transactions')
    logging.info(f'number of periods and transactions in the data: {n_days} periods (unit={unit}s), {n_samples} transactions')
